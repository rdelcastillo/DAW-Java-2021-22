...
public class EncryptFileCaesarCipher {

  public static void main(String[] args) {
    exitIfWrongArguments(args);
    
    String fileNameToEncrypt = args[0];
    String encryptedFileName = getNameTargetFile(args);
    int key = readKey();

    try {
      CaesarCipher.encrypt(fileNameToEncrypt, encryptedFileName, key);
      System.out.println("Creado " + encryptedFileName);
    } 
    catch (NoSuchFileException e) {
      exitWithError("No puedo abrir " + fileNameToEncrypt, READING_ERROR);
    } 
    catch (FileNotFoundException e) {
      exitWithError("Error al escribir en " + encryptedFileName, WRITING_ERROR);
    } 
    catch (IOException e) {
